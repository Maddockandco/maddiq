...
export default function SalesInvoices({ clientId }: { clientId: string }) {
  ...
  useEffect(() => {
    ...
    if (!trimmed) { setInvoiceNumberDuplicate(false); return }
    ...
    const timer = setTimeout(async () => {
      let query = supabase
        ...
        .neq('status', 'void')
      if (editingInvoiceId) query = query.neq('id', editingInvoiceId)

      const { count } = await query
      setInvoiceNumberDuplicate((count || 0) > 0)
      setCheckingInvoiceNumber(false)
    }, 400)

    return () => clearTimeout(timer)
  }, [invoiceNumberInput, clientId, editingInvoiceId])
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <input
                ...
                className={`${inputClass} ${invoiceNumberDuplicate ? 'border-red-300 focus:ring-red-300' : ''}`}
              />
              {invoiceNumberDuplicate && (
                <p className="text-xs text-red-600 mt-1">This invoice number is already in use for this client.</p>
              )}
            </div>
            <div>
          ...
          <div className="flex gap-3">
            <ActionDropdown
              loading={saving}
              loadingLabel="Saving..."
              primary={{ key: 'save_draft', label: editingInvoiceId ? 'Save Changes' : 'Save as Draft', onClick: () => handleCreate('none'), disabled: invoiceNumberDuplicate }}
              options={[
                { key: 'approve', label: 'Approve', onClick: () => handleCreate('approve'), disabled: invoiceNumberDuplicate },
                { key: 'approve_email', label: 'Approve and Email', onClick: () => handleCreate('approve_and_email'), disabled: invoiceNumberDuplicate },
              ]}
            />
