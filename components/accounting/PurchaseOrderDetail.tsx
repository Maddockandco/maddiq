...
export default function PurchaseOrderDetail({ clientId, orderId }: { clientId: string; orderId: string }) {
  ...
  async function handleConvert() {
    ...
    const { error: convertErr } = await supabase.rpc('convert_purchase_order_to_bill', {
      p_order_id: orderId,
    ...
    await logAudit({
      ...
      description: `Converted "${order.order_number}" to a purchase bill`,
    })
  ...
  return (
    <div className="space-y-6">
      <button
        onClick={() => router.push(`/accounting/${clientId}/purchase-orders`)}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            <p className="text-xs text-gray-400 uppercase tracking-wider">Purchase Order</p>
        ...
        {order.status === 'converted' && (
          <div className="mt-6 pt-6 border-t border-gray-100">
            <p className="text-sm text-gray-400">✓ This order has been converted to a purchase bill</p>
          </div>
        )}
        ...
        {showConvert && (
          <div className="mt-6 pt-6 border-t border-gray-100 space-y-3">
            ...
            <div className="flex items-end gap-4 flex-wrap">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Supplier's bill number (optional)</label>
                <input type="text" value={billNumber} onChange={(e) => setBillNumber(e.target.value)} className={inputClass} />
              </div>
              <div>
      ...
      <TransactionAuditTrail entityType="purchase_order" entityId={orderId} />
    </div>
  )
}
