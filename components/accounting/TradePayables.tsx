...
export default function TradePayables({ clientId }: { clientId: string }) {
  ...
  const enriched = bills.map((bill) => {
    ...
    const daysOverdue = Math.floor((today.getTime() - new Date(bill.due_date).getTime()) / (1000 * 60 * 60 * 24))
    return { ...bill, outstanding, daysOverdue, bucket: getBucket(daysOverdue) }
  }).filter((bill) => bill.outstanding > 0.005)

  const byContact: Record<string, { name: string; bills: any[]; buckets: Record<AgingBucket, number>; total: number }> = {}
  for (const bill of enriched) {
    const key = bill.contact_id
    if (!byContact[key]) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-brand-dark uppercase tracking-wider">Trade Payables (Aged Payables)</h3>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
          <p className="text-sm text-gray-400">No outstanding bills — nice and clean</p>
        </div>
      ) : view === 'summary' ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100">
                      <th className="text-left px-6 py-2 text-xs font-semibold text-gray-500">Bill #</th>
                  ...
                  <tbody>
                    {c.bills.sort((a: any, b: any) => b.daysOverdue - a.daysOverdue).map((bill: any) => (
                      <tr key={bill.id} className="border-b border-gray-50">
                        <td className="px-6 py-2 text-brand-dark font-mono">{bill.bill_number || '—'}</td>
                        <td className="px-6 py-2 text-gray-600">{new Date(bill.due_date).toLocaleDateString('en-GB')}</td>
                        <td className="px-6 py-2 text-right text-gray-600">{bill.daysOverdue > 0 ? bill.daysOverdue : 0}</td>
                        <td className="px-6 py-2">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${BUCKET_COLORS[bill.bucket as AgingBucket]}`}>{BUCKET_LABELS[bill.bucket as AgingBucket]}</span>
                        </td>
                        <td className="px-6 py-2 text-right font-medium text-brand-dark">£{bill.outstanding.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
