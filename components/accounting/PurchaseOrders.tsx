...
export default function PurchaseOrders({ clientId }: { clientId: string }) {
  ...
  async function handleSave() {
    ...
    await logAudit({
      ...
      action: 'created',
      newData: order,
      description: `Created purchase order "${orderNumber}" for £${total.toFixed(2)}`,
    })
  ...
  if (contacts.length === 0) return (
    <div className="bg-white rounded-2xl shadow-sm p-12 text-center border border-gray-200">
      ...
      <p className="text-gray-400 text-xs">Add a supplier in Contacts first before creating purchase orders</p>
    </div>
  )
                ...
                <>
                  <tr key={o.id} className="border-b border-gray-100">
                    <td className="px-6 py-3 text-sm">
                      <button onClick={() => router.push(`/accounting/${clientId}/purchase-orders/${o.id}`)} className="font-mono text-brand-dark font-medium hover:underline">
                        {o.order_number}
                      </button>
                    </td>
                    ...
                    <td className="px-6 py-3 text-right space-x-2 whitespace-nowrap">
                      ...
                      {o.status === 'converted' && (
                        <span className="text-xs text-gray-400">Bill created</span>
                      )}
                    </td>
                  </tr>
                      ...
                      <td colSpan={6} className="px-6 py-4">
                        <div className="flex items-end gap-4 flex-wrap">
                          ...
                          <div>
                            ...
                            <input type="text" value={billNumber} onChange={(e) => setBillNumber(e.target.value)} placeholder="e.g. their invoice ref"
