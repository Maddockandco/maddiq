    ...
    <div className="space-y-6">
      ...
      {creating && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
          <h3 className="text-sm font-semibold text-brand-dark uppercase tracking-wider">New Purchase Bill</h3>
          <p className="text-xs text-gray-400 -mt-2">Creating directly — this won't be linked to a Purchase Order</p>
              ...
              {bills.map((bill) => (
                <>
                  <tr key={bill.id} className="border-b border-gray-100">
                    <td className="px-6 py-3 text-sm font-mono text-gray-600">{bill.bill_number || '—'}</td>
                    ...
                    <td className="px-6 py-3 text-right">
                      ...
                      {bill.journal_entry_id && bill.status !== 'draft' && (
