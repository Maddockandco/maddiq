    ...
    <div className="space-y-6">
      {!adding && !editing && (
        <div className="flex justify-end">
          <button onClick={() => setAdding(true)} className="bg-brand-dark text-white font-semibold px-5 py-2.5 rounded-xl text-sm hover:bg-opacity-90 transition">
            + Add Engagement
          </button>
        </div>
      )}
      ...
      {engagements.length === 0 && !adding ? (
        ...
        !adding && !editing && <EngagementTable engagements={engagements} onEdit={setEditing} />
      )}
    </div>
  )
}
