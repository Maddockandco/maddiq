...
export default function ClientTaxEditForm({ clientId }: { clientId: string }) {
  ...
  async function handleSave() {
    ...
    // Only update these fields if not payroll only
    if (!isPayrollOnly) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* CH and CT sections — hidden from payroll managers */}
      {!isPayrollOnly && (
        <>
      ...
      {/* Banking and Billing — hidden from payroll */}
      {!isPayrollOnly && (
        <>
