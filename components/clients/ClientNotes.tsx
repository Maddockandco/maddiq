...
export default function ClientNotes({ clientId }: { clientId: string }) {
  ...
  async function fetchNotes() {
    const { data } = await supabase
      .from('notes')
      .select('id, content, created_at, engagement_id, firm_users(full_name, user_id)')
  ...
  // Split notes into parent notes and replies
  // For now all notes are top-level — replies are notes added via NoteReply
  // We identify replies by checking engagement_id used as parent_id marker
  const parentNotes = notes.filter(n => !n.engagement_id)
  const replies = notes.filter(n => n.engagement_id)
        ...
        <div className="space-y-4">
          {parentNotes.map((note) => (
            <NoteItem
              ...
              clientId={clientId}
              firmId={firmId}
              replies={replies.filter(r => r.engagement_id === note.id)}
