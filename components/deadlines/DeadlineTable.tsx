        ...
        <table className="w-full min-w-[700px]">
          <thead>
            <tr className="bg-brand-dark">
              ...
              <th className="text-left px-6 py-4 text-xs font-semibold text-white uppercase tracking-wider">Description</th>
              <th className="text-left px-6 py-4 text-xs font-semibold text-white uppercase tracking-wider">Period End</th>
              ...
              return (
                <tr key={deadline.id} className={`border-b border-gray-100 hover:bg-amber-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                  <td className="px-6 py-4">
                    <span className="font-medium text-brand-dark text-sm">{deadline.clients?.name || '—'}</span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm text-gray-500">
                      {deadline.period_end ? new Date(deadline.period_end).toLocaleDateString('en-GB') : '—'}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div>
                      <span className={`text-sm font-medium ${isOverdue ? 'text-red-600' : isDueSoon ? 'text-amber-600' : 'text-brand-dark'}`}>
